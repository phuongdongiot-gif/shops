...
function shopping_customize_register($wp_customize)
{
    ...
    for ($i = 1; $i <= 3; $i++) {
        ...
        $wp_customize->add_control('benefit_' . $i . '_title', array(
            'label' => 'Tiêu đề Cam Kết ' . $i,
            'section' => 'shopping_benefits_section',
            'type' => 'text',
        ));
    ...
    $wp_customize->add_control('footer_links', array(
        'label' => 'Dải Liên Kết',
        'section' => 'shopping_footer_bottom',
        'type' => 'textarea',
    ));
    ...
    $wp_customize->add_control('footer_info_title', array(
        ...
        'section' => 'shopping_footer_info_section',
        'type' => 'text',
    ));
    ...
    $wp_customize->add_control('footer_info_content', array(
        'label' => 'Nội dung HTML cột thông tin',
        'section' => 'shopping_footer_info_section',
        'type' => 'textarea',
    ));
    ...
    $wp_customize->add_control('footer_policy_content', array(
        'label' => 'Nội dung HTML cột Chính sách',
        'section' => 'shopping_footer_info_section',
        'type' => 'textarea',
    ));
...
function shopping_dynamic_course_layout()
{
    if (is_post_type_archive('lp_course') || is_tax('course_category') || is_tax('course_tag')) {
        ?>
        <style>
            /* === Force Grid Layout: Luôn dùng grid, không phụ thuộc vào trạng thái list/grid của LP === */
            ...
            /* Ẩn nút chuyển đổi Grid/List */
            .lp-courses-bar .courses-switch-view,
            .lp-courses-bar .courses-switch-layout {
                display: none !important;
            }

            /* Toàn bộ vùng archive Courses full width */
            .lp-archive-courses {
                width: 100% !important;
...
// =========================================================
// ẨN SIDEBAR WIDGET MẶC ĐỊNH CỦA LEARNPRESS
// (.lp-archive-courses-sidebar và .course-sidebar-secondary)
...
function shopping_suppress_lp_widget_sidebars($is_active, $sidebar_id) {
    ...
    // Ẩn 2 sidebar widget area mặc định của LP
    $lp_sidebars = ['archive-courses-sidebar', 'course-sidebar'];
    if (in_array($sidebar_id, $lp_sidebars, true)) {
        return false;
    }
...
function shopping_lp_archive_layout_css() {
    if (is_post_type_archive('lp_course') || is_tax('course_category') || is_tax('course_tag')) {
        ?>
        <style>
            /* Force layout 2 cột (content + sidebar) cho trang /courses */
            .lp-content-area {
                display: flex !important;
                gap: 2rem !important;
...
// =========================================================
// TẮT BÌNH LUẬN TRÊN TẤT CẢ TRANG LEARNPRESS (/courses & /courses/slug)
// =========================================================


/**
 * 1. Đóng comments cho post type lp_course ở cấp độ WordPress core
...
/**
 * 2. Số comments luôn = 0 với lp_course
...
/**
 * 3. Xóa action comments_template của LearnPress khỏi single course
...
function shopping_collapsible_description()
{
    if (is_product()) {
        ...
        <style>
            ...
            .woocommerce-Tabs-panel--description.collapsed {
                max-height: 400px;
...
function shopping_floating_action_buttons()
{
    ...
    <style>
        ...
        @media (max-width: 767px) {
            ...
            /* Lift up so it doesnt overlap sticky bar */
        }
    </style>
    ...
    <div class="fab-container">
        <?php if ($messenger): ?>
            <a href="<?php echo esc_url($messenger); ?>" target="_blank" rel="noopener" class="fab-btn fab-mess"
                title="Messenger">
                <svg viewBox="0 0 36 36">
                    <path
                        d="M18 2C9.16 2 2 8.7 2 17c0 4.67 2.3 8.85 5.8 11.66V34l5.31-2.92c1.55.43 3.18.66 4.89.66 8.84 0 16-6.7 16-15S26.84 2 18 2zm1.63 20.12l-4.13-4.4-8.06 4.4 8.82-9.39 4.14 4.4 8.05-4.4-8.82 9.39z" />
                </svg>
            </a>
        <?php endif; ?>

        <?php if ($zalo): ?>
            <a href="<?php echo esc_url($zalo); ?>" target="_blank" rel="noopener" class="fab-btn fab-zalo" title="Zalo"
                style="color: white; font-family: sans-serif; font-weight: bold; font-size: 14px;">
                Zalo
            </a>
        <?php endif; ?>
    ...
    <?php if (function_exists('is_product') && is_product()): ?>
        <div class="sticky-mobile-bar active">
            <a href="tel:<?php echo esc_attr($tel); ?>"
                class="flex-1 bg-[#f3f4f6] text-gray-800 text-center py-2.5 rounded-lg border border-gray-200 font-bold flex items-center justify-center gap-2 hover:bg-gray-200 transition-colors">
            ...
            <a href="<?php echo esc_url($zalo ? $zalo : ($messenger ? $messenger : 'tel:' . $tel)); ?>" target="_blank"
                rel="noopener"
                class="flex-1 bg-[#ea580c] text-white text-center py-2.5 rounded-lg font-bold flex items-center justify-center gap-2 shadow-sm hover:opacity-90 transition-opacity">
...
function shopping_ajax_live_search()
{
    ...
    if ($search_query->have_posts()) {
        while ($search_query->have_posts()) {
            ...
            if (!$thumb)
                $thumb = 'https://ui-avatars.com/api/?name=' . urlencode(get_the_title()) . '&background=f3f4f6&color=9ca3af';
