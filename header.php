...
<body <?php body_class('antialiased font-base text-body overflow-x-hidden relative min-h-screen flex flex-col'); ?>>
    ...
    <div id="page" class="site flex-1 flex flex-col">
        <a class="skip-link screen-reader-text sr-only"
            href="#primary"><?php esc_html_e('Skip to content', 'shopping'); ?></a>
        ...
        <div class="bg-primary text-white py-1.5 hidden md:block">
            <div class="container mx-auto px-6 flex justify-between items-center text-[13px]">
                <div class="flex items-center gap-6">
                    ...
                    <a href="mailto:<?php echo esc_attr($top_email); ?>"
                        class="flex items-center gap-1.5 hover:text-gray-200 transition">
                    ...
                    <a href="tel:<?php echo esc_attr(str_replace(' ', '', $top_phone)); ?>"
                        class="flex items-center gap-1.5 font-bold hover:text-gray-200 transition text-[14px]">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="flex items-center gap-3">
                    <a href="#" aria-label="Facebook" class="hover:text-gray-200 transition"><svg class="w-4 h-4"
                            fill="currentColor" viewBox="0 0 24 24">
                            <path
                                d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.469h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.469h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                        </svg></a>
                    <a href="#" aria-label="Twitter" class="hover:text-gray-200 transition"><svg class="w-4 h-4"
                            fill="currentColor" viewBox="0 0 24 24">
                            <path
                    ...
                    <a href="mailto:<?php echo esc_attr($top_email); ?>" aria-label="Email"
                        class="hover:text-gray-200 transition"><svg class="w-4 h-4" fill="currentColor"
                            viewBox="0 0 24 24">
                            <path
            ...
            <div class="container mx-auto px-6 flex flex-col lg:flex-row items-center justify-between gap-6">
                ...
                <div class="header-benefits hidden lg:flex items-center gap-6 pl-8 flex-1 justify-end">
                    ...
                    for ($i = 1; $i <= 3; $i++) {
                        ...
                        if ($i > 1)
                            echo '<div class="absolute -left-4 top-1/2 -translate-y-1/2 h-8 w-px bg-gray-300"></div>'; // Separator line like in image
        ...
        <!-- 3. BOTTOMBAR NAV -->
        <header id="masthead"
            class="site-header relative lg:sticky top-0 z-50 bg-white/95 backdrop-blur shadow-sm transition-all duration-300">
            <div
                class="container mx-auto px-4 md:px-6 min-h-[56px] flex flex-wrap items-center justify-between gap-y-3 py-3 lg:py-0">
...
<<<<<<< HEAD
                ...
                <div
                    class="live-search-container relative w-full lg:w-64 xl:w-80 order-3 lg:order-2 lg:ml-auto mt-1 lg:mt-0">
                    <form role="search" method="get" class="search-form relative z-10 block"
                        action="<?php echo esc_url(home_url('/')); ?>">
                        <?php if (class_exists('WooCommerce')): ?>
                            <input type="hidden" name="post_type" value="product" />
                        <?php endif; ?>
                        <input type="search" id="live-search-input"
                            class="search-field w-full pl-4 pr-10 py-2.5 text-[13px] text-gray-800 bg-[#f3f4f6] border border-transparent rounded-full focus:bg-white focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all shadow-inner"
                            placeholder="<?php echo esc_attr__('Tìm kiếm sản phẩm...', 'shopping'); ?>"
                            value="<?php echo get_search_query(); ?>" name="s" autocomplete="off" />
                        <button type="submit"
                            class="search-submit absolute right-0 top-0 h-full px-4 text-gray-500 hover:text-primary transition-colors flex items-center justify-center">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <div id="live-search-results"
                        class="absolute top-full left-0 right-0 lg:right-0 lg:left-auto mt-2 bg-white rounded-xl shadow-[0_10px_40px_rgb(0,0,0,0.12)] border border-gray-100 overflow-hidden z-[100] hidden flex-col w-full lg:min-w-[300px] max-h-[70vh] overflow-y-auto transform origin-top transition-all scale-y-95 opacity-0">
                        <div class="p-4 text-center text-sm text-gray-500 loading-indicator hidden">
                            <svg class="animate-spin h-5 w-5 mx-auto text-primary" xmlns="http://www.w3.org/2000/svg"
                        ...
                        <a href="#" id="view-all-results"
                            class="hidden block w-full text-center py-3 bg-gray-50 text-[13px] font-bold text-primary hover:bg-orange-50 transition-colors border-t border-gray-100 uppercase tracking-widest">
                ...
                <div
                    class="header-actions-right w-auto flex items-center justify-end gap-4 lg:gap-6 order-2 lg:order-3 lg:ml-6">
                    <!-- Polylang Switcher -->
                    <?php if (function_exists('pll_the_languages')): ?>
                        <ul
                            class="polylang-switcher flex items-center gap-3 list-none m-0 p-0 lg:pr-6 lg:border-r border-gray-200">
                    ...
                    <!-- Cart / Tư vấn -->
                    <?php if (class_exists('WooCommerce')): ?>
                        <a id="shopping-cart-trigger"
                            class="header-cart relative flex items-center text-dark font-medium group"
                            href="<?php echo esc_url(wc_get_cart_url()); ?>" title="Xem danh sách cần tư vấn">
                            <svg class="header-cart-icon w-6 h-6 text-gray-700 group-hover:text-primary transition-colors"
                            ...
                            <span class="ml-2 text-[14px] hidden lg:block group-hover:text-primary transition-colors">Danh
                                sách</span>
                            <div
                                class="header-cart-count absolute -top-2 lg:-right-2 -right-2 bg-primary text-white text-xs font-bold w-5 h-5 flex items-center justify-center rounded-full shadow-sm">
                                <?php echo WC()->cart->get_cart_contents_count(); ?></div>
                        </a>
                    <?php endif; ?>
                </div>
