...
<nav id="site-navigation" class="main-navigation w-full lg:w-auto static lg:relative">
	...
	<button id="menu-toggle-btn"
		class="menu-toggle lg:hidden flex items-center gap-2 px-3 py-2 border rounded text-dark border-gray-300 hover:text-primary hover:border-primary transition-colors bg-white font-heading"
	...
	<div id="primary-menu-wrap"
		class="hidden absolute left-0 right-0 top-full mt-4 lg:mt-0 bg-white lg:bg-transparent shadow-lg lg:shadow-none p-4 lg:p-0 z-50 lg:relative lg:flex lg:items-center">
		<?php
		wp_nav_menu(
			array(
				...
				'fallback_cb' => function () {
					...
					if (taxonomy_exists('product_cat')) {
						...
						$product_categories = get_terms(array('taxonomy' => 'product_cat', 'hide_empty' => false, 'parent' => 0));
						if (!empty($product_categories) && !is_wp_error($product_categories)) {
							// Update to use opacity/visibility instead of display to avoid WP core CSS conflicts
							echo '<ul class="absolute left-0 lg:-left-16 xl:-left-32 top-full mt-2 w-[280px] sm:w-[500px] md:w-[700px] bg-white border border-gray-100 shadow-xl rounded-md p-4 invisible opacity-0 pointer-events-none group-hover:visible group-hover:opacity-100 group-hover:pointer-events-auto transition-all duration-300 transform translate-y-2 group-hover:translate-y-0 z-[60] grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-2">';
							foreach ($product_categories as $category) {
								if (!in_array($category->slug, array('uncategorized', 'chua-phan-loai')) && $category->name !== 'Chưa phân loại') {
									echo '<li><a href="' . esc_url(get_term_link($category)) . '" class="block px-4 py-2.5 text-[13px] font-semibold text-gray-700 hover:bg-orange-50 hover:text-primary transition-colors capitalize rounded-md border border-transparent hover:border-orange-100">' . esc_html($category->name) . '</a></li>';
								}
							}
							echo '</ul>';
						}
					...
					if (post_type_exists('product')) {
						...
						if ($product_query->have_posts()) {
							...
							while ($product_query->have_posts()) {
								...
								echo '<div class="absolute inset-0 bg-black/60 opacity-0 group-hover/item:opacity-100 transition-opacity duration-300 flex items-center justify-center p-2"><span class="text-white text-[12px] leading-snug font-semibold text-center line-clamp-3">' . get_the_title() . '</span></div>';
								echo '</a>';
							}
							wp_reset_postdata();
							echo '</div>';
                        ...
                        // ---- Khóa học (trước Tin tức) ----
                        if (class_exists('LearnPress')) {
                            $courses_link = get_post_type_archive_link('lp_course') ?: home_url('/courses/');
                            echo '<li><a href="' . esc_url($courses_link) . '" class="hover:text-primary transition-colors flex items-center gap-1">'
